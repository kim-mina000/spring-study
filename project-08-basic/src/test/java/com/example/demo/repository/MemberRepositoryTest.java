...
public class MemberRepositoryTest {
	...
	@Test
	public void 회원등록() {
		Member member = new Member("user1","1234","냥");
	...
	@Test
	public void 모든회원삭제() {
		Board board = Board.builder().writer(Member.builder().id("user1").build()).build();
	...
	public void 회원등록2() {
		for(int i = 0; i < 30; i++) {
			Member member = Member.builder()
																			.id("user"+i)
																			.name(String.valueOf(Math.floor(Math.random()*100)%25))
																			.password("1234")
																			.build();
			memberRepository.save(member);
		}
