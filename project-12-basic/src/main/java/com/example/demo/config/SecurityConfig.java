...
public class SecurityConfig {
	...
	public  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.addFilterBefore( new ApiCheckFilter(arr, jwtUtil(), customUserDetailsService()), // 주소 유틸 인증서비스 
				UsernamePasswordAuthenticationFilter.class ); // 우선순위
...
//			.requestMatchers("/board/*").hasRole("USER")
			...
			.and()
			.csrf().disable() // csrf 비활성화 토큰을 사용하니까 세선은 사용안함
			.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
		// 기본적으로 폼 로그인 방식을 사용하면 제이세션이라는 쿠키가 생성되서 브라우저에 저장됨
		// 저장된 쿠키로 상태를 유지함 지금 api 형태로 개발하기 때문에 해당 로직이 필요없음 -> 설정 해제 STATELESS
